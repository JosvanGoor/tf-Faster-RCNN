...
def _detect_boxes(data_directory, names, sess, tf_inputs, tf_outputs, thresh, det_dir, key, vis):
    ...
    for i in tqdm(range(num_images)):
        ...
        if vis:
            ...
            output.append([dets,cls])
    # Save Detections
    with open(det_file, 'wb') as f:
        pickle.dump(all_boxes, f)
    det_file = det_dir + key + '_boxes_classes.pkl'
    with open(det_file, 'wb') as f:
...
def _plot_patch(ax, bbox, prob, class_name, color):
    ...
    width = bbox[2] - bbox[0]
    rect = patches.Rectangle((bbox[0], bbox[1]), width, height, linewidth=1, edgecolor=color, facecolor='none')
    ax.add_patch(rect)
